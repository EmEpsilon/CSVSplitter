...
2. CSVファイルのドロップが完了したら、「2.処理対象のCSVファイル(ファイルを取り込んだら、下の分析ボタンを押すこと)」の右下にある、「分析」ボタンを押してください。
...
4. 「変換条件」の指定を行います。
  ...
  * 複数のCSVファイルを統合する
    * 複数ファイルを1つのCSVに統合した上で処理します。
      * 対象ファイル全て同じ項目と文字コードのCSVファイルの必要があります。
  * CSV出力最大行数
...
5. 4の「変換条件」を指定後
  * 出力フォルダを指定します。
  * 最期に「変換開始」ボタンを押します。
  * 「変換が完了しました。」のメッセージが表示されたら、ファイル出力が完了です。「出力先フォルダを開く」を押すと、出力先のフォルダを開くことができます。
